...
const BookCartItem = ({ cart }) => {
  ...
  const book = books.filter(book => book.id === id); //USE MEMO
    ...
    <div className="book-item mt-4">
      <div className="row">
        ...
        <div className="col-lg-3 col-md-3 col-sm-12 col-12">
          ...
          <h5>Price: <i class="fa fa-inr"></i>{price}</h5>
          <h5>Total Price: <i class="fa fa-inr"></i> {(price * newQuantity).toFixed(2)}</h5>
        </div>
        ...
        <div className="col-lg-3 col-md-3 col-sm-12 col-12">
          <button onClick={handleCartRemove} className="btn btn-danger">
          Remove Item
          </button>
        </div>
      </div>
